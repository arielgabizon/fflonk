...
$D^{(2)}=(D^{(1)})^{(1)}$ and $D^{(3)}=(D^{(2)})^{(1)}$ respectively.
 ...
 For a vector $\f\in D^t$, we refer to the elements of \f by $f_i,0\leq i < t$. Similarly for $\ff\in D^{(2)}$, we refer to the elements of \ff, which are vectors over $D$,  by $\f_i, 0\leq i < |\ff|$; and the elements of the \set{\f_i} by \sett{f_{i,j}}{i\in \lt{|\ff|},j\in \lt{|\f_i|}} 
$\polys\defeq \F[X]$.
...
For $\ff\in\polys^{(2)}$ and $\ZZ \in \rects$ with $|\ff|=|\ZZ|$,  we denote by $\ff(\ZZ)\in \cubes$ the element  $\ff(\ZZ)\defeq  \vecc{\f_i(\Z_i)}{i\in\lt{|\ff|}}$.
